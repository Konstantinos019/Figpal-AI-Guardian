  ...
  return (
    <div className="my-2">
      <button
        ...
        className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-md transition-colors w-full text-left overflow-hidden min-w-0 cursor-pointer ${isActive
      ...
      {(open || isActive) && (
        <div className={`mt-1 ml-5 px-3 py-2 rounded text-xs leading-relaxed border-l-2 ${isActive ? "border-violet-500/30 text-violet-200/70" : "border-white/10 text-white/40"
          }`}>
          {text}
        </div>
      )}
    ...
    <div className="my-3 flex flex-wrap gap-2">
      {choices.map((choice, i) => (
        <button
          ...
          disabled={disabled}
          className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${disabled
...
function ToolCallBlock({ toolName, input, output, isError }: { toolName: string; input?: Record<string, unknown>; output?: unknown; isError?: boolean }) {
  ...
  return (
    <div className="my-2">
      <button
        onClick={() => setOpen(!open)}
        className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-md transition-colors w-full text-left overflow-hidden min-w-0 cursor-pointer ${isError
...
function ToolCallProgress({ toolName }: { toolName: string }) {
  ...
  return (
    <div className={`my-2 px-3 py-2 rounded text-xs font-mono flex items-center gap-2 ${isStalled ? "bg-amber-500/10 border border-amber-500/20 text-amber-300/70" : "bg-white/5 text-white/50"
      }`}>
  ...
  const transport = useMemo(
    () =>
      new DefaultChatTransport({
        ...
        body: () => ({
          figmaMcpUrl: figmaOAuthRef.current ? "https://mcp.figma.com/mcp" : figmaMcpUrlRef.current,
          figmaAccessToken: figmaAccessTokenRef.current,
          codeProjectPath: codeProjectPathRef.current,
          figmaOAuth: figmaOAuthRef.current,
            ...
            <div>
              ...
              <div className="p-3 bg-white/5 border border-white/10 rounded-md">
                <p className="text-[10px] text-white/40 leading-relaxed mb-2">
                  Access Figma canvas via Chrome Extension bridge.
                </p>
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${isBridgeConnected ? "bg-emerald-500" : "bg-red-500"} animate-pulse`} />
                  <span className="text-[11px] font-medium text-white/70">
                    {isBridgeConnected ? "Extension Active" : "Extension Inactive"}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        ...
        <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-3 sm:px-4 py-4 sm:py-6">
          ...
          {messages.map((m) => (
            ...
            >
              <div
                className={`max-w-full sm:max-w-[80%] rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 text-sm leading-relaxed ${m.role === "user"
                  ? "glass-msg-user"
                  : "glass-msg-ai"
                  }`}
              >
